...
def Database():
    ...
    cursor.execute("SELECT * FROM `admin` WHERE `username` = 'admi' AND `password` = 'admi'")#contraseña aqui  se puede cambiar ,solo al principio la primwera ves q se ejecuta 
    if cursor.fetchone() is None:
        cursor.execute("INSERT INTO `admin` (username, password) VALUES('admi', 'admi')")
        conn.commit()
...
def ShowLoginForm():
    global loginform
    loginform = Toplevel()
    loginform.title("Sistema De Stock /Account Login")
    width = 600
    height = 500
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
